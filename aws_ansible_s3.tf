resource "aws_s3_bucket_object" "db_ansible_playbook" {
  ...
  etag = md5(data.template_file.db_ansible_playbook.rendered)
}
...
resource "aws_s3_bucket_object" "ssl_ansible_playbook" {
  ...
  etag = md5(data.template_file.ssl_ansible_playbook.rendered)
}
...
resource "aws_s3_bucket_object" "update_ansible_playbook" {
  ...
  etag = md5(data.template_file.update_ansible_playbook.rendered)
}